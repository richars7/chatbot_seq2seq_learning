...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 14 10:40:16 2018
...
lines = open('/Users/savita/desktop/cornell movie-dialogs corpus/movie_lines.txt',encoding='utf-8' , errors='ignore').read().split('\n')
conversations = open('/Users/savita/desktop/cornell movie-dialogs corpus/movie_conversations.txt',encoding='utf-8' , errors='ignore').read().split('\n')
...
#creating two dictoniers that maps the question words and answers word to unique integers          
threshold = 20 
questionswords2int = {}
word_number = 0
...
#adding the last tokens to these two dictoneires 
...
#construct the encoder RNN layer 
def encoder_rnn(rnn_inputs, rnn_size, num_layers, keep_prob , sequence_length ):
    lstm = tf.contrib.rnn.BasicLSTM(rnn_size)
...
def decoder_rnn(decoder_embedded_input , decoder_embeddings_matrix , encoder_state , num_words, sequence_length , rnn_size , num_layers , word2int , keep_prob , batch_size):
    with tf.variable_scope("decoding") as decoding_scope:
        ...
        output_function = lambda x: tf.contrib.layers.fully_connectedy_connected(x, num_words,None , scope= decoding_scope, weights_initializer= weights , biases_intializer = biases)
...
training_predictions , test_predictions = seq2seq_model(tf.reverse(inputs),[-1],targets , keep_prob, batch_size , sequence_length, len(answersword2int) , len(questionswords2int), encoding_embedding_size, decoding_embedding_size, rnn_size, num_layers, questionswords2int )

#setting up the loss error , the optimizer and gradient clippping 
...
#spliiting the questions and answers into training and validation sets 
